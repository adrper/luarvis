...
// --- Ícones SVG Internos (Máxima compatibilidade e performance) ---
...
const predictNextWindow = (lat, lon, isCurrentlyVisible) => {
  ...
  if (isCurrentlyVisible) {
    for (let i = 0; i < 48; i++) {
      ...
      const visibleNow = pos.altitude > 0 && hour >= 6 && hour <= 20;
      if (!visibleNow) break;
  ...
  if (windowStart && windowEnd) {
    ...
    return `entre ${formatTime(windowStart)} e ${formatTime(windowEnd)} do dia ${formatDate(windowStart)}`;
  }
...
const App = () => {
  ...
  const [lastCheck, setLastCheck] = useState({ visible: false, reason: "Iniciando..." });
  const [nextVisibility, setNextVisibility] = useState("Aguardando localização...");
  ...
  useEffect(() => {
    const styleId = 'luarvis-styles';
    if (!document.getElementById(styleId)) {
      const style = document.createElement('style');
      style.id = styleId;
      style.innerHTML = `
        html, body { background-color: #020617 !important; margin: 0; padding: 0; height: 100%; width: 100%; color: white; font-family: -apple-system, system-ui, sans-serif; }
        ...
        .card-visible { background-color: rgba(37, 99, 235, 0.1); border-color: #3b82f6; box-shadow: 0 10px 40px -10px rgba(59, 130, 246, 0.3); }
        ...
        .btn-round { padding: 12px; background-color: #1e293b; border-radius: 100px; border: none; color: #94a3b8; cursor: pointer; display: flex; }
        .quiz-option { width: 100%; padding: 16px; border-radius: 18px; border: 1px solid #1e293b; background-color: #1e293b; color: #e2e8f0; text-align: left; cursor: pointer; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; font-size: 14px; font-weight: 600; }
        .quiz-correct { background-color: rgba(34, 197, 94, 0.2) !important; border-color: #22c55e !important; color: #bbf7d0 !important; }
        .quiz-wrong { background-color: rgba(239, 68, 68, 0.2) !important; border-color: #ef4444 !important; color: #fecaca !important; }
        .feedback-box { background-color: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 20px; padding: 20px; margin-top: 16px; }
        .text-label { font-size: 10px; text-transform: uppercase; font-weight: 900; letter-spacing: 0.15em; color: #94a3b8; margin-bottom: 6px; }
        .text-title { font-size: 20px; font-weight: 800; margin: 0; line-height: 1.2; letter-spacing: -0.02em; color: white; }
        .btn-next { background-color: rgba(59, 130, 246, 0.2); color: #60a5fa; border: none; padding: 12px 24px; border-radius: 100px; font-weight: 900; font-size: 11px; cursor: pointer; display: flex; align-items: center; gap: 8px; text-transform: uppercase; }
        .alert-box { background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.4); border-radius: 20px; padding: 16px; display: flex; align-items: center; gap: 12px; }
        .footer { text-align: center; color: #475569; padding: 30px 0; font-size: 9px; text-transform: uppercase; font-weight: 800; letter-spacing: 0.1em; width: 100%; }
    ...
    if (localStorage.getItem('luarvis_last_visit_date') !== today) {
      ...
      localStorage.setItem('luarvis_visit_count', newCount.toString());
  ...
  const requestLocation = () => {
    ...
    } else { setLoading(false); setLocationError(true); }
  };
  ...
  const checkVisibility = async () => {
    ...
    try {
      ...
      setLastCheck({ 
        visible: isVisible, 
        reason: hour < 6 || hour > 20 ? "Fora do horário (06h-20h)" : (pos.altitude <= 0 ? "Abaixo do horizonte" : (data.current.cloud_cover >= 40 ? "Céu muito nublado" : "Céu aberto"))
      });
    ...
    } catch (e) { setNextVisibility("Erro ao calcular"); }
  };

  useEffect(() => { if (location) checkVisibility(); }, [location, notifPermission]);

  const showQuiz = !lastCheck?.visible || visitCount >= 2;

  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#020617', color: 'white', fontFamily: 'sans-serif' }}>
      <MoonIcon className="animate-pulse" style={{ color: '#60a5fa', marginBottom: '16px' }} size={60} />
      <p style={{ fontWeight: '600', color: '#94a3b8', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Consultando os astros...</p>
    </div>
  );
      ...
      <div className="app-container">
        {locationError && (
          <div className="alert-box">
            <AlertIcon style={{ color: '#ef4444' }} size={24} />
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: '13px', fontWeight: '900', margin: 0, color: '#fecaca' }}>LOCALIZAÇÃO DESATIVADA</p>
              <p style={{ fontSize: '11px', margin: 0, color: 'rgba(252,165,165,0.8)' }}>Ative o GPS para o LuarVis funcionar.</p>
            </div>
            <button onClick={requestLocation} style={{ backgroundColor: '#ef4444', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', fontSize: '11px' }}>ATIVAR</button>
          </div>
        )}

        <header className="header">
          <div className="flex-center" style={{ gap: '10px' }}>
            <MoonIcon style={{ color: '#3b82f6' }} size={32} />
            <h1 style={{ margin: 0, fontSize: '26px', fontWeight: '900', letterSpacing: '-0.06em' }}>LuarVis</h1>
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            {notifPermission !== 'granted' && (
              <button onClick={requestNotificationPermission} className="btn-round"><BellIcon size={22} /></button>
            )}
            <button onClick={() => location ? checkVisibility() : requestLocation()} className="btn-round"><RefreshIcon size={22} /></button>
          </div>
        </header>
        ...
        <section className={`card ${lastCheck?.visible ? 'card-visible' : ''}`}>
          <div className="flex-center">
            <div className={`icon-box ${lastCheck?.visible ? 'icon-blue' : 'icon-slate'}`}>
              {lastCheck?.visible ? <EyeIcon size={36} /> : <EyeOffIcon size={36} />}
            </div>
            <div>
              <p className="text-label">Consigo ver a lua agora?</p>
              <h2 className="text-title">
                {!location ? 'Aguardando GPS...' : (lastCheck?.visible ? 'Sim! A lua deve estar visível.' : 'Não, lua escondida :(')}
              </h2>
              {location && <p style={{ fontSize: '13px', color: '#64748b', margin: '6px 0 0', fontWeight: '500' }}>{lastCheck?.reason}</p>}
            </div>
          </div>
        </section>
        ...
        <section className="card">
          <div style={{ position: 'relative', zIndex: 2 }}>
            <div className="flex-center" style={{ gap: '8px', marginBottom: '14px' }}>
              <CalendarIcon style={{ color: '#3b82f6' }} size={18} />
              <h3 className="text-label" style={{ margin: 0 }}>Próxima Visibilidade</h3>
            </div>
            <p style={{ fontSize: '18px', fontWeight: '800', margin: 0, color: '#f1f5f9' }}>
              {!location ? 'Ative o GPS para prever' : nextVisibility}
            </p>
          </div>
          <MoonIcon style={{ position: 'absolute', right: '-30px', bottom: '-30px', opacity: 0.05, color: 'white' }} size={140} />
        </section>

        {showQuiz && (
          <section className="card" style={{ padding: 0 }}>
            <div style={{ backgroundColor: 'rgba(30,41,59,0.5)', padding: '18px', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <TrophyIcon style={{ color: '#eab308' }} size={18} />
              <span className="text-label" style={{ margin: 0, color: '#e2e8f0' }}>Testar conhecimentos</span>
            </div>
            <div style={{ padding: '24px' }}>
              <p style={{ fontSize: '18px', fontWeight: '800', textAlign: 'center', marginBottom: '26px', lineHeight: '1.4' }}>
                ...
                {QUIZ_CARDS[cardIndex].options.map((opt, i) => {
                  ...
                  return (
                    <button 
                      ...
                      style={answered && i !== selectedOption && i !== QUIZ_CARDS[cardIndex].correct ? {opacity: 0.4} : {}}
                    >
                      <span>{opt}</span>
              ...
              {answered && (
                <div className="feedback-box">
                  ...
                  <p style={{ fontSize: '14px', margin: 0, color: '#cbd5e1', lineHeight: '1.5' }}>{QUIZ_CARDS[cardIndex].explanation}</p>
        ...
        <footer className="footer">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '6px' }}>
            <MapPinIcon size={12} />
            <span>{location ? `COORD: ${location.lat.toFixed(2)}, ${location.lon.toFixed(2)}` : 'SINAL GPS PENDENTE'}</span>
          </div>
          <div>LuarVis v2.4 • Observação Urbana</div>
        </footer>
      </div>
    </div>
  );
